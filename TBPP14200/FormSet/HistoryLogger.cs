...
namespace TBPP14200
{
    public partial class HistoryLogger : Form
    {
        ...
        public HistoryLogger()
        {
            ...
            //+By Max 2019/09/11
            LoggerFactory.Logger.Add(null, EnLoggerType.EnLog_OP, "Date\tTime\tType\tContent");
        ...
        public void AddTitle(EnLoggerType type, string title)
        {
            ...
            dc = new DataColumn("Package");
            dt.Columns.Add(dc);
            dc = new DataColumn("Lot");
            dt.Columns.Add(dc);

            dc = new DataColumn("Content");
            dt.Columns.Add(dc);
            dc = new DataColumn("Error code");
            dt.Columns.Add(dc);
            dc = new DataColumn("Mode");
            dt.Columns.Add(dc);
            dc = new DataColumn("Date of occurrence");
            dt.Columns.Add(dc);
            dc = new DataColumn("Time of occurrence");
            dt.Columns.Add(dc);
            dc = new DataColumn("Duration (sec)");
            dt.Columns.Add(dc);
        }
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                dv.RowFilter = "Type = '" + comboBox1.Text + "'";
            }
            dv.Sort = "Time DESC";
        ...
        private void btnLoggerViewer_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (hCCDHandle == (IntPtr)0)
                {
                    ...
                    SInfo.Arguments = SYSPara.LogPath;
                    Process.Start(SInfo);
                }
        ...
        private void btnBackup_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    MessageBox.Show("請安裝專案ProVDLL資料夾中的DotNetCode安裝程式以利一鍵備份工具正常運行", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                //    return;
                //}
        ...
        private bool IsDotnetCoreInstalled()
        {
            const string subkey = @"SOFTWARE\WOW6432Node\Microsoft\Updates\.NET Core\Microsoft ASP.NET Core 5.0.1 - Shared Framework (x86)\";
            bool bRet = true;
            using (var ndpKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).OpenSubKey(subkey))
            {
                if (ndpKey != null && ndpKey.GetValue("InstallerVersion") != null)
                {
                    bRet = true;
                }
                else
