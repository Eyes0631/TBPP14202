...
namespace TBPP14200
{
    public partial class MainF1 : Form
    {
        ...
        */
        ...
        private void UserLogin()
        {
            ...
            while (!blogin)
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, string.Format("{0} login", lbUserType.Text));
                }
        ...
        private void PageChange(object sender, EventArgs e)
        {
            ...
            switch (tag)
            {
                case 1: //自動操作
                    ...
                    AllFormHide();
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Auto」");

                    SYSPara.SysRun = false;
                    FormSet.mMSS.M_Stop();
                    SYSPara.SysState = StateMode.SM_PACKAGELOAD_RESET;
                    SYSPara.SystemProductionMode = PRODUCTION_MODE.NONE;
                    FormSet.mPackage.FileName_Auto = "";

                    using (LotInfoForm LotInfo = new LotInfoForm())
                    {
                        DialogResult ret = LotInfo.ShowDialog();
                ...
                case 2: //手動操作
                    AllFormHide();
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Manual」");
                ...
                case 3: //產品設定
                    AllFormHide();
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Package」");
                ...
                case 4: //通用設定
                    AllFormHide();
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Option」");
                    ...
                    break;
                case 5: //切換語系
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Language」");
                ...
                case 6: //History
                    AllFormHide();
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「History」");
        ...
        {
            SYSPara.LogSay(EnLoggerType.EnLog_OP, string.Format("{0} logout", lbUserType.Text));
        ...
        {
            SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Exit program」");

            if (!SYSPara.SysRun)
            {
                DialogResult result = MessageBox.Show("Do you want to leave?", "Notice", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                switch (result)
                {
        ...
        {
            SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Alarm Reset」");  //警報重置
            SYSPara.MusicOn = false;
            SYSPara.ErrorStop = false;
            SYSPara.Alarm.ClearAll();
        ...
        private void MainF1_Load(object sender, EventArgs e)
        {
            ...
            tv_LeftKit.DataVCL = FormSet.mMainFlow.td_Left_KitShuttle;
            tv_RightKit.DataVCL = FormSet.mMainFlow.td_Right_KitShuttle;
            tv_HDT_TopBoard.DataVCL = FormSet.mMainFlow.td_HDT_BIB_A;
            tv_HDT_BottomBoard.DataVCL = FormSet.mMainFlow.td_HDT_BIB_B;
            tv_HDT_TopBF.DataVCL = FormSet.mMainFlow.td_HDT_BF_A;
            tv_HDT_BottomBF.DataVCL = FormSet.mMainFlow.td_HDT_BF_B;
            tv_TopBowlFeeder.DataVCL = FormSet.mMainFlow.td_Top_BFM;
            tv_BottomBowlFeeder.DataVCL = FormSet.mMainFlow.td_Bottom_BFM;
            tv_LeftBoard.DataVCL = FormSet.mMainFlow.td_Left_Board;
            tv_RightBoard.DataVCL = FormSet.mMainFlow.td_Right_Board;
            tv_ClamSheel.DataVCL = FormSet.mMainFlow.td_CHM;
            tv_Transfer.DataVCL = FormSet.mMainFlow.td_TRM;
            tv_LeftLoadUnload.DataVCL = FormSet.mMainFlow.td_Unload;
            tv_RightLoadUnload.DataVCL = FormSet.mMainFlow.td_Load;

            tv_LeftKit.BinColorBuf.Clear();
            tv_LeftKit.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_RightKit.BinColorBuf.Clear();
            tv_RightKit.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_HDT_TopBoard.BinColorBuf.Clear();
            tv_HDT_TopBoard.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_HDT_BottomBoard.BinColorBuf.Clear();
            tv_HDT_BottomBoard.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_HDT_TopBF.BinColorBuf.Clear();
            tv_HDT_TopBF.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_HDT_BottomBF.BinColorBuf.Clear();
            tv_HDT_BottomBF.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_TopBowlFeeder.BinColorBuf.Clear();
            tv_TopBowlFeeder.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_BottomBowlFeeder.BinColorBuf.Clear();
            tv_BottomBowlFeeder.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_RightBoard.BinColorBuf.Clear();
            tv_RightBoard.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_ClamSheel.BinColorBuf.Clear();
            tv_ClamSheel.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_Transfer.BinColorBuf.Clear();
            tv_Transfer.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_LeftLoadUnload.BinColorBuf.Clear();
            tv_LeftLoadUnload.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);

            tv_RightLoadUnload.BinColorBuf.Clear();
            tv_RightLoadUnload.BinColorBuf.AddRange(tv_LeftBoard.BinColorBuf);
            //安全門機制改善
        ...
        private int GetAllFailBinQty()
        {
            int iRet = 0;
            JBINQTY Bin2Qty = GlobalDefine.OEE.GetBinQty(BinDefine.Bin2);

            for (int i = 2; i < 32; i++)
            {
                JBINQTY Qty = GlobalDefine.OEE.GetBinQty((BinDefine)i);
                iRet += Qty.Completed;
            }
            return iRet;
        }
        ...
        private void tmUIRefresh_Tick(object sender, EventArgs e)
        {
            ...
            #region 使用者修改
            //tiitle
            lb_LBoardID.Text = SYSPara.StageA_BIBID;
            lb_RBoardID.Text = SYSPara.StageB_BIBID;
            lb_LotID.Text = SYSPara.NewLotInfo.LotNumber;
            lb_RunMode.Text = SYSPara.SystemProductionMode.Equals(PRODUCTION_MODE.LOAD) ? "LOAD" : "UNLOAD";
            ...
            JBINQTY PassBinQty = GlobalDefine.OEE.GetBinQty(BinDefine.Bin1);
            //JBINQTY Bin2Qty = GlobalDefine.OEE.GetBinQty(BinDefine.Bin2);
            int FailQty = GetAllFailBinQty();
            lb_LoadProcessQty.Value = string.Format("{0:0}", LoadQty.Process);
            lb_LoadCompletedQty.Value = string.Format("{0:0}", LoadQty.Completed);
            lb_LoadMissingQty.Value = string.Format("{0:0}", LoadQty.Missing);
            lb_UnloadProcessQty.Value = string.Format("{0:0}", UnLoadQty.Process);
            lb_UnloadPassQty.Value = string.Format("{0:0}", PassBinQty.Completed);
            //lb_UnloadFailQty.Value = string.Format("{0:0}", Bin2Qty.Completed);
            lb_UnloadFailQty.Value = string.Format("{0:0}", FailQty);
            lb_BibCount.Value = GlobalDefine.OEE.BoardCount.ToString();
            if (SYSPara.RunSecond > 0)
            {
                lb_UPH.Value = string.Format("{0:0.00}", (((double)(LoadQty.Completed + UnLoadQty.Completed) / (double)SYSPara.RunSecond) * 3.6));
            }
            else
            {
                lb_UPH.Value = string.Format("{0:0.00}", 0);
            }
