...
namespace TBPP14200
{
    public partial class PMSettingF : Form
    {
        ...
        public PMSettingF(bool bEditMode)
        {
            ...
            DataTable dtMileage = new DataTable("ItemMileage");
            DataColumn colMileage = new DataColumn("保養軸名");
            dtMileage.Columns.Add(colMileage);
            colMileage = new DataColumn("保養里程", System.Type.GetType("System.Int32"));
            dtMileage.Columns.Add(colMileage);
            colMileage = new DataColumn("保養提示", System.Type.GetType("System.String"));
            dtMileage.Columns.Add(colMileage);
            colMileage = new DataColumn("里程設定", System.Type.GetType("System.Int32"));
            dtMileage.Columns.Add(colMileage);
            colMileage = new DataColumn("需保養", System.Type.GetType("System.Boolean"));
            dtMileage.Columns.Add(colMileage);
            colMileage = new DataColumn("保養狀態", System.Type.GetType("System.String"));
            dtMileage.Columns.Add(colMileage);
            dsPM.Tables.Add(dtMileage);
            ...
            if (System.IO.File.Exists("PMSetting.xml"))
            {
                ...
                //Mileage Region
                dv = new DataView(dsPM.Tables["ItemMileage"]);

                if (m_bEditMode)
                {
                    dgvMileage.DataSource = dv;
                    dgvMileage.Columns[0].ReadOnly = true;
                    dgvMileage.Columns[1].ReadOnly = true;
                    dgvMileage.Columns[2].ReadOnly = true;
                    dgvMileage.Columns[3].Visible = false;
                    dgvMileage.Columns[4].Visible = false;
                    dgvMileage.Columns[5].Visible = false;

                    DataGridViewComboBoxColumn pmActionComboBox = Create_ComboBox();
                    dgvMileage.Columns.Add(pmActionComboBox);

                    for (int i = 0; i < dsPM.Tables["ItemMileage"].Rows.Count; ++i)
                    {
                        dgvMileage.Rows[i].Cells[5].Value = "跳過此次";
                    }
                ...
                else
                {
                    for (int i = 0; i < dsPM.Tables["ItemMileage"].Rows.Count; ++i)
                    {
                        String mtrName = (String)dsPM.Tables["ItemMileage"].Rows[i][0];

                        ProVLib.Motor mtr = ProVLib.MotorList.Motors.Find(m => m.Name == mtrName); 

                        dsPM.Tables["ItemMileage"].Rows[i]["需保養"] = false;
                        int d = (int)dsPM.Tables["ItemMileage"].Rows[i][1];
                        //if (d.Subtract(dtNow).Ticks < 0)
                        //{
                        //    dsPM.Tables["ItemMileage"].Rows[i]["需保養"] = true;
                        //    m_HasPMItem = true;
                        //}
                    }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            bool bNewAddItemMile = false;
            if (dsPM.Tables["ItemMileage"].Rows.Count == 0)
                bNewAddItemMile = true;

            if (pmItem.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                if (Item.PMCycle == ePMCycle.eMileage)
                {
                    DataRow row = dsPM.Tables["ItemMileage"].NewRow();
                    row["保養軸名"] = Item.ItemName;
                    row["保養里程"] = Item.PMMile;
                    row["保養提示"] = Item.PMHint;
                    row["里程設定"] = Item.PMMile;
                    row["需保養"] = false;
                    row["保養狀態"] = "新增保養";
                    dsPM.Tables["ItemMileage"].Rows.Add(row);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            switch (SYSPara.LoginMode)
            {
                case 0:
                    {
                        ...
                        sLoginUser = FormSet.mUserLoginEx.PMUserName; //20210701 kent
                ...
                case 1:
                    {
                        sLoginUser = SYSPara.mSecret.NowUser.Name;
                    }
                    break;
            }
            ...
            for (int i = 0; i < dgvMileage.Rows.Count; ++i)
            {
                if (String.IsNullOrEmpty((String)dgvMileage.Rows[i].Cells["狀態"].Value) && !(String.Compare((String)dgvMileage.Rows[i].Cells["保養狀態"].Value, "新增保養") == 0))
                {
                    dgvMileage.Rows[i].Cells["保養狀態"].Value = "未設定";
                }
            }
            ...
            for (int i = 0; i < dgvMileage.Rows.Count; ++i)
            {
                if (String.Compare((String)dgvMileage.Rows[i].Cells["保養狀態"].Value, "完成保養") == 0)
                {
                    String strLog = String.Format("使用者[{0}]完成[{1}]的預防保養項目", sLoginUser, (String)dgvMileage.Rows[i].Cells["保養軸名"].Value);
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, strLog);

                    dgvPM.Rows[i].Cells["保養里程"] = dgvPM.Rows[i].Cells["里程設定"];

                }
                else if (String.Compare((String)dgvMileage.Rows[i].Cells["保養狀態"].Value, "跳過此次") == 0)
                {
                    String strLog = String.Format("使用者[{0}]跳過[{1}]的預防保養項目", sLoginUser, (String)dgvMileage.Rows[i].Cells["保養軸名"].Value);
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, strLog);

                    dgvMileage.Rows[i].Cells["保養里程"] = dgvMileage.Rows[i].Cells["里程設定"];

                }
                else if (String.Compare((String)dgvMileage.Rows[i].Cells["保養狀態"].Value, "週期重置") == 0)
                {
                    String strLog = String.Format("使用者[{0}]重置[{1}]的預防保養項目", sLoginUser, (String)dgvMileage.Rows[i].Cells["保養軸名"].Value);
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, strLog);

                    dgvMileage.Rows[i].Cells["保養里程"] = dgvMileage.Rows[i].Cells["里程設定"];
                }
                else if (String.Compare((String)dgvMileage.Rows[i].Cells["保養狀態"].Value, "新增保養") == 0)
                {
                    String strLog = String.Format("使用者[{0}]新增[{1}]的預防保養項目", sLoginUser, (String)dgvMileage.Rows[i].Cells["保養軸名"].Value);
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, strLog);
                    dgvMileage.Rows[i].Cells["保養狀態"].Value = "完成保養";
                }
                //else if (String.Compare((String)dgvPM.Rows[i].Cells["保養狀態"].Value, "未設定") == 0)
                //{
                //    String strLog = String.Format("[{0}]的預防保養項目未設定保養狀態!", (String)dgvPM.Rows[i].Cells["保養項目"].Value);
