    ...
    public partial class MSS : Form
    {
        ...
        private void ArmProcess(Control module, ArmDT pArm)
        {
            ...
            if (errcode.Length >= 3)
            {
                ...
                {
                    pArm.ModuleID += mid;
                    pArm.ErrorCode = errcode.Substring(errcode.Length - 2, 2);
                }
            }
        ...
        //維修模式前置設定
        private void M_MaintenanceReset()
        {
            string name = "";
            try
            {
                foreach (BaseModuleInterface module in FormSet.ModuleList)
                {
                    name = module.Name;
                    if (module.GetUseModule())
                        module.MaintenanceReset();
                }
                name = "MainFlow";
                FormSet.mMainFlow.MaintenanceReset();
            }
            catch (Exception ex)
            {
                SYSPara.Alarm.Say("E0520", name);
                MessageBox.Show(ex.StackTrace.ToString());
            }
        }
        private void M_MaintenanceRun()
        {
            string name = "";
            try
            {
                foreach (BaseModuleInterface module in FormSet.ModuleList)
                {
                    name = module.Name;
                    if (module.GetUseModule())
                        module.MaintenanceRun();
                }
                name = "MainFlow";
                FormSet.mMainFlow.MaintenanceRun();
            }
            catch (Exception ex)
            {
                SYSPara.Alarm.Say("E0521", name);
                MessageBox.Show(ex.StackTrace.ToString());
            }
        }
        ...
        private FlowChart.FCRESULT FC_HOME_Run()
        {
            ...
            switch (SYSPara.SysState)
            {
                case StateMode.SM_INIT_RUN:
                    if (SYSPara.SysRun)
                    {
                        if (M_Home())
                        {
                            ...
                            SYSPara.LogSay(EnLoggerType.EnLog_OP, "Homing finish"); //歸零完成
                ...
                case StateMode.SM_ABORT:
                    ...
                    SYSPara.LogSay(EnLoggerType.EnLog_OP, "Cancel the homing process"); //取消歸零流程
        ...
        private FlowChart.FCRESULT FC_MANUAL_Run()
        {
            switch (SYSPara.SysState)
            {
                case StateMode.SM_MAINTENANCE_RUN:
                    SYSPara.ShowAlarm("i", 522);
            ...
            switch (SYSPara.SysState)
            {
                ...
                case StateMode.SM_CANCEL:
                    ...
                    {
                        SYSPara.SysState = StateMode.SM_MAINTENANCE_RUN;
                        return FlowChart.FCRESULT.CASE1;
                    }
                    else
                    {
        ...
        private FlowChart.FCRESULT FC_Maintenance_Run()
        {
            if (MotionCall != "")
                return FlowChart.FCRESULT.IDLE;
        ...
        private FlowChart.FCRESULT FC_AUTO_RESET_Run()
        {
            try
            {
                ...
                //bool b1 = (bool)SYSPara.CallProc("DPM", "UseOneDiepakLotMode");
                bool b1 = false;
                if (!b1)
                {
                ...
                case StateMode.SM_INIT_RESET:
                    {
                        ...
                        SYSPara.LogSay(EnLoggerType.EnLog_OP, "Enter the homing process");    //進入歸零流程
        ...
        {
            SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Mode&IO scan」");    //掃描模組IO
        ...
        {
            SYSPara.LogSay(EnLoggerType.EnLog_OP, "User Click「Language scan」"); //語系掃描
        ...
        private FlowChart.FCRESULT FC_IDLE_Run()
        {
            ...
            switch (SYSPara.SysState)
            {
                case StateMode.SM_PACKAGELOAD_RESET:
                    return FlowChart.FCRESULT.CASE1;
                case StateMode.SM_MANUAL_RESET:
                case StateMode.SM_MAINTENANCE_RESET:
                    if (SYSPara.SysState == StateMode.SM_MAINTENANCE_RESET)
                    {
                        SYSPara.RunMode = RunModeDT.MAINTENANCE;
                        SYSPara.ManualControlIO = false;
                        MotionCall = "M_MaintenanceReset";
                        SYSPara.SysState = StateMode.SM_MAINTENANCE_RUN;
                    }
                    else
                    {
        ...
        private void btn_進入維修模式_Click(object sender, EventArgs e)
        {
            if (SYSPara.IsAutoMode)
                return;

            SYSPara.LogSay(EnLoggerType.EnLog_OP, "使用者按下「進入維修模式」");

            SYSPara.SysRun = false;
            FormSet.mMSS.M_Stop();
            SYSPara.SysState = StateMode.SM_MAINTENANCE_RESET;
        }

        private void btn_退出維修模式_Click(object sender, EventArgs e)
        {
            if (SYSPara.SysState == StateMode.SM_MAINTENANCE_RUN)
                SYSPara.SysState = StateMode.SM_CANCEL;
        }

        private void btn權限掃描_Click(object sender, EventArgs e)
        {
            SYSPara.LogSay(EnLoggerType.EnLog_OP, "使用者按下「權限掃描」");
            SYSPara.mSecret.SaveToFile();
            MessageBox.Show("使用者權限儲存完成");
        }

        private void btn使用者管理_Click(object sender, EventArgs e)
        {
            UserManagment frm = new UserManagment();
            frm.LoadUser();
            frm.ShowDialog();
            frm.Dispose();
        }
