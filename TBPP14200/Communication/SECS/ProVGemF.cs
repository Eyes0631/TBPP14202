...
namespace TBPP14200
{
    ...
    public partial class ProVGemF : Form
    {
        ...
        {
            if ((CurCommState == ProVGEMCommFSM.StateID.COMMUNICATING || SECSEngine.SpoolActive) &&
        ...
        private long proVSECSEngine1_SECSEvent(ProVTool.EVENTID EvtID, ProVTool.EventData pEventData)
        {
            switch (EvtID)
            {
                ...
                case ProVTool.EVENTID.EVENT_REPLY_TIMEOUT:
                    ...
                    break;
                case EVENTID.EVENT_SPOOLACTIVE:
                    {
                        //Stream Type 1 Can't be Spooled
                        if (pEventData.S != 1)
                        {
                            SnFnData canSpoolSnFn = SnFnList.Find(x => (x.SType == pEventData.S && x.FType == pEventData.F));
                            if (canSpoolSnFn != null && canSpoolSnFn.EnableSpool)
                            {
                                SpoolData spool = new SpoolData(pEventData.S, pEventData.F, pEventData.W_Bit, pEventData.RawData, pEventData.ulSystemBytes);
                                SpoolList.Add(spool);
                            }
                        }
                    }
                    break;
                case EVENTID.EVENT_DEVICEIDERR:
                    {
                        //Unrecognized Device ID
                        List<byte> sb = new List<byte>();
                        SECSEngine.SendHSMSMessage(9, 1, false, sb.ToArray());
                    }
                    break;
                case EVENTID.EVENT_DATAERROR:
                    {
                        //Illegal Data
                        List<byte> sb = new List<byte>();
                        SECSEngine.SendHSMSMessage(9, 7, false, sb.ToArray());
                    }
                    break;
            }
            return default(long);
